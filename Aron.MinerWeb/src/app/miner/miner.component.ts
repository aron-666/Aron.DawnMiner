...
export class MinerComponent implements OnInit {
  ...
  refreshCaptcha() {
    this.http.get('/api/Miner/RefreshCaptcha', { withCredentials: true })
      .subscribe({
        error: (error) => {
  ...
  applyCaptcha() {
    const answer = (<HTMLInputElement>document.getElementById('puzzelAns')).value;
    this.http.post('/api/Miner/ApplyCaptcha', { ans: answer }, { withCredentials: true })
      .subscribe({
        next: () => {
          (<HTMLInputElement>document.getElementById('puzzelAns')).value = '';
        },
        error: (error) => {
          console.error('Error applying captcha:', error);
        }
      });
  }
}
